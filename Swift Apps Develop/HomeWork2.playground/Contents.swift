...
func  myFunc (a1: Int?, _ b1: String?, c1: Bool?) {
    if let a1, let b1, let c1 { //Тут сначала забыл let во втором и третьем случае
...
func arrayDoubleToTwoBoolTuple (a: [Double]) -> (Bool, Bool) {
    return (a.count > 5 ? true : false, a.contains(10.0) ? true : false) //тут сначала хотел перебирать массив через цикл и сравнивать каждый элемент равен ли 10, но не получилось сравнивать 10 и 10.0 из-за разных типов, rounded почему-то применялось с ошибкой, в тоге спроил у чатгпт как найти ходит ли число в double массиве он мне подсказал про conteins и тогда я решил написать через 2 тернарных оператор. Но все равно хотелось бы попробовать решить эту задачу через for-in и rounded, чтобы понять как это сделать правильно.
}
